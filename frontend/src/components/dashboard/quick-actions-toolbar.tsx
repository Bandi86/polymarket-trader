...
function QuickAction({
  ...
  const colorMap = {
    ...
    green: "border-emerald-500/20 hover:border-emerald-500/40 hover:bg-emerald-500/5 text-emerald-400",
...
export function QuickActionsToolbar({ showLabels = true, compact = false }: QuickActionsToolbarProps) {
  const { tradingMode, sseHealth, latency } = useAppStore();
  const isDemo = tradingMode === "demo";

  const latencyAvg = latency.avg;
  const latencyStatus =
    latencyAvg > 0 && latencyAvg < 200 ? "green" : latencyAvg > 0 && latencyAvg < 500 ? "amber" : "red";
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1.5 pr-2 border-r border-white/10">
          <span className={`text-[9px] font-bold uppercase ${isDemo ? "text-indigo-400" : "text-emerald-400"}`}>
            {isDemo ? "Demo" : "Live"}
          </span>
        </div>
        ...
        {/* Latency */}
        {latencyAvg > 0 && (
          <div className={`flex items-center gap-1 px-2 border-r border-white/10 text-${latencyStatus}-400`}>
      ...
      <div className="flex items-center gap-1.5 text-[10px] text-zinc-500">
        <div className={`h-1.5 w-1.5 rounded-full ${sseHealth.connected ? "bg-emerald-400" : "bg-red-400"}`} />
